...
from settings import REDIS_CLIENT, PSQL_CLIENT, PSQL_DB, REDIS_RETENTION_PERIOD,  _logging
...
class DBUpdater:
    def __init__(self):
        ...
        self.__redis_session = aioredis.from_url(redis_url, password=REDIS_CLIENT.PASSWORD,
        ...
        self.__psql_session = sessionmaker(psql_engine, expire_on_commit=False, class_=AsyncSession)
    ...
    async def get_latest_prices(self):
        ...
        ts = await self.__redis_session.execute_command("TS.MRANGE", current_time - REDIS_RETENTION_PERIOD * 60000 , current_time, 'FILTER', 'type=trading_instruments')
        self.logger.info(f"Got instrument prices from Redis for the last {REDIS_RETENTION_PERIOD} minutes")
        df_lst = []
    ...
    async def save_latest_prices(self, df_latest_prices):
        if df_latest_prices.shape[0] == 0: # no new data
            self.logger.info(f"No data in Redis for the last {REDIS_RETENTION_PERIOD} minutes")
            return
